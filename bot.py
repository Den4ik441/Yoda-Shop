...
with sqlite3.connect('database.db') as conn:
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS numbers (
        ...
        TG_NUMBER INTEGER DEFAULT 1,  -- Новый столбец
        STATUS TEXT
    )''')
...
def admin_view_groups(call):
    user_id = call.from_user.id


    with db.get_db() as conn:
        cursor = conn.cursor()
        # Получаем все группы и их модераторов
        cursor.execute('''
            SELECT DISTINCT p.GROUP_ID, p.ID
            FROM personal p
            WHERE p.TYPE = 'moder' AND p.GROUP_ID IS NOT NULL
        ''')
    ...
    if not groups:
        bot.edit_message_text(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=create_back_to_main_markup()
        )
        return

    text = "👥 <b>Список групп:</b>\n\n"
    markup = types.InlineKeyboardMarkup()
    for group_id, moderator_id in groups:
        cursor.execute('SELECT COUNT(*) FROM numbers n LEFT JOIN personal p ON p.ID = n.ID_OWNER WHERE n.STATUS = "отстоял" AND (p.GROUP_ID = ? OR p.GROUP_ID IS NULL)', (group_id,))
        successful_count = cursor.fetchone()[0]
        text += f"📊 Группа {group_id} (Модератор: {moderator_id})\n"
        text += f"✅ Успешных номеров: {successful_count}\n"
        markup.add(types.InlineKeyboardButton(f"📋 Статистика группы {group_id}", callback_data=f"view_group_stats_{group_id}"))
        text += "—\n"
...
# НОМЕРА КОТОРЫЕ ПОКАЗЫВАЮТСЯ У  МОДЕРАТОРОВ:
...
#ВСЕ НОМЕРА КОТОРЫЕ ПОКАЗЫВАЮТСЯ У АДМИНИСТАТОРА + СТАТИТСИКА ГРУПП
            ...
            with db.get_db() as conn:
                ...
                for number, owner_id, take_date, moderator_id, group_chat_id in numbers:
                    try:
                        ...
                        if time_diff >= 10:  # Если прошло 10 минут или больше
                            # Сбрасываем номер в очередь
                            cursor.execute('''
                                UPDATE numbers
                                SET TAKE_DATE = "0",
                                    MODERATOR_ID = NULL,
                            ...
                            try:
                                bot.send_message(
                                    owner_id,
                                    f"❌ Ваш номер {number} был не обработан модератором в течение 10 минут.\n"
                                    "📱 Он снова в очереди.",
                                    reply_markup=markup
                                )
                                print(f"Сообщение отправлено владельцу {owner_id}")
                            except Exception as e:
                                print(f"Ошибка при отправке сообщения владельцу {owner_id}: {e}")
                            ...
                            if moderator_id:
                                ...
                                try:
                                    target_chat_id = group_chat_id if group_chat_id else moderator_id
                                    bot.send_message(
                                        target_chat_id,
                                        f"❌ Номер {number} был возвращён в очередь, так как вы не обработали его в течение 10 минут.",
                                        reply_markup=markup_mod
                                    )
                                except Exception as e:
                                    print(f"Ошибка при отправке сообщения модератору {moderator_id}: {e}")
                        ...
                        if work_time >= hold_time:
                            ...
                            if moderator_id:
                                ...
                                try:
                                    ...
                                    bot.send_message(
                                        target_chat_id,
                                        f"✅ Номер {number} успешно отстоял {hold_time} минут.\n"
                                        f"🟢 Статус: отстоял",
                                        reply_markup=markup_mod
                                    )
                                except Exception as e:
...
def send_verification_code(call):
    try:
        ...
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, TG_NUMBER FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            owner_id, tg_number = result
            tg_number = tg_number if tg_number is not None else 1
        
        print(f"send_verification_code: number={number}, group_chat_id={group_chat_id}, tg_number={tg_number}")  # Отладка
...
def process_verification_code_input(message, number, moderator_id, group_chat_id, original_chat_id, original_message_id, tg_number=None):
    try:
        user_id = message.from_user.id

        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT TG_NUMBER FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.send_message(
                    message.chat.id,
                    f"❌ Номер {number} не найден в базе!",
                    parse_mode='HTML'
                )
                return
            tg_number = result[0] if result[0] is not None else 1
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("❌ Нет, изменить", callback_data=f"change_code_{number}_{group_chat_id}")
        )
        ...
        if user_id in active_code_requests:
            for other_number in active_code_requests[user_id].copy():
                if other_number != number:
                    try:
                        other_msg_id = active_code_requests[user_id][other_number]
                        bot.delete_message(user_id, other_msg_id)
                    except Exception as e:
                        print(f"Ошибка при удалении сообщения для номера {other_number}: {e}")
...
def confirm_code(call):
    try:    
        ...
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT MODERATOR_ID, GROUP_CHAT_ID, TG_NUMBER FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            moderator_id, stored_chat_id, tg_number = result
            tg_number = tg_number if tg_number is not None else 1
...
def change_code(call):
    try:
        print(f"Callback data received: {call.data}")  # Отладочный вывод
        parts = call.data.split("_")
        if len(parts) < 3:
            bot.answer_callback_query(call.id, "❌ Неверный формат данных! (Недостаточно частей)")
            return
        
        number = parts[2]
        group_chat_id = int(parts[3]) if len(parts) > 3 else call.message.chat.id
        
        print(f"Extracted: number={number}, group_chat_id={group_chat_id}")  # Отладочный вывод
        
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT MODERATOR_ID, TAKE_DATE, TG_NUMBER FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            moderator_id = result[0] if result[0] else call.from_user.id
            take_date = result[1]
            tg_number = result[2] if result[2] is not None else 1
        ...
        if take_date not in ("0", "1"):
            ...
            bot.send_message(
                call.message.chat.id,
                f"📱 ТГ {tg_number}\n"
                f"📱 Номер: {number}\n✉️ Запрос нового кода отправлен владельцу.",
                parse_mode='HTML'
            )
            ...
            bot.answer_callback_query(call.id, "❌ Номер ещё не взят для обработки!")
...
def number_active(call):
    try:
        print(f"Callback data received in number_active: {call.data}")  # Отладочный вывод
        parts = call.data.split("_")
        if len(parts) < 3:
            bot.answer_callback_query(call.id, "❌ Неверный формат данных!")
            return
        ...
        number = parts[2]
        tg_number = int(parts[3]) if len(parts) > 3 else 1
        
        print(f"Extracted in number_active: number={number}, tg_number={tg_number}")  # Отладочный вывод
        ...
        with db.get_db() as conn:
            ...
            if status == "активен":
                bot.answer_callback_query(call.id, "✅ Номер уже активен!")
                return
                
            current_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute('UPDATE numbers SET STATUS = ?, CONFIRMED_BY_MODERATOR_ID = ?, TAKE_DATE = ? WHERE NUMBER = ?', 
                          ('активен', call.from_user.id, current_date, number))
            conn.commit()
...
def back_to_confirm(call):
    try:
        print(f"Callback data received: {call.data}")  # Отладочный вывод
        prefix = "back_to_confirm_"
        if not call.data.startswith(prefix):
            bot.answer_callback_query(call.id, "❌ Неверный формат данных!")
            return
        
        number = call.data[len(prefix):]
        print(f"Extracted number: {number}")  # Отладочный вывод
        
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, VERIFICATION_CODE, TAKE_DATE FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            ...
            if not result:
                ...
                bot.send_message(
                    call.message.chat.id,
                    f"❌ Номер {number} больше недоступен (возможно, он был удалён или отклонён).\nПожалуйста, получите новый номер.",
                    reply_markup=markup
                )
                return
            
            owner_id, code, take_date = result

        tg_number = 1  # Значение по умолчанию
        # Если код уже есть, показываем этап подтверждения
        if code and take_date not in ("0", "1"):
            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton("✅ Да, встал", callback_data=f"number_active_{number}_{tg_number}"),
                types.InlineKeyboardButton("❌ Нет, изменить", callback_data=f"change_code_{number}_{call.message.chat.id}_{tg_number}")
            )
            ...
            bot.send_message(
                call.message.chat.id,
                f"📱 ТГ {tg_number}\n📱 Код по номеру {number}\nКод: {code}\n\nВстал ли номер?",
                reply_markup=markup
            )
        elif owner_id and take_date not in ("0", "1"):
            try:
                bot.delete_message(call.message.chat.id, call.message.message_id)
            except Exception as e:
                print(f"Ошибка при удалении сообщения: {e}")
            ...
            bot.send_message(
                call.message.chat.id,
                f"📱 ТГ {tg_number}\n📱 Новый номер для проверки: <code>{number}</code>\n"
                "Ожидайте код от владельца или отметьте номер как невалидный.",
                parse_mode='HTML',
                reply_markup=markup
            )
            ...
            bot.send_message(
                call.message.chat.id,
                f"❌ Номер {number} больше недоступен или владелец не найден.\nПожалуйста, получите новый номер.",
                reply_markup=markup
            )
...
def init_db():
    with db.get_db() as conn:
        cursor = conn.cursor()
        # Проверяем, существует ли столбец GROUP_CHAT_ID
