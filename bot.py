...
@bot.callback_query_handler(func=lambda call: call.data == "group_statistics")
def show_group_statistics(call):
    if call.from_user.id not in config.ADMINS_ID:
    ...
    if not groups:
        ...
        text = "<b>📊 Выберите группу для просмотра статистики:</b>"
        markup = types.InlineKeyboardMarkup()
...
def show_group_details(call):
    ...
    with db.get_db() as conn:
        ...
        if not group:
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="group_statistics"))
            markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=markup)
            return
        ...
        cursor.execute('SELECT COUNT(*) FROM personal WHERE GROUP_ID = ?', (group_id,))
        member_count = cursor.fetchone()[0]
        
        # Подсчитываем успешные номера
        cursor.execute('''
            SELECT COUNT(*) 
            FROM numbers n
            JOIN personal p ON n.ID_OWNER = p.ID
            WHERE p.GROUP_ID = ? AND n.STATUS = 'активен'
        ''', (group_id,))
        successful_numbers = cursor.fetchone()[0]
        
        # Получаем все номера группы
        cursor.execute('''
    ...
    if not numbers:
        text += "📭 Нет номеров в этой группе."
    
    # Пагинация
    items_per_page = 20  # Количество номеров на страницу
    total_pages = (len(numbers) + items_per_page - 1) // items_per_page
    start_idx = (page - 1) * items_per_page
    end_idx = start_idx + items_per_page
    page_numbers = numbers[start_idx:end_idx]
    
    # Добавляем номера на текущую страницу
    for number, take_date, shutdown_date, status in page_numbers:
        text += (
            f"📞 <code>{number}</code>\n"
            f"📊 Статус: {status if status else 'Не указан'}\n"
        )
    ...
    markup.add(types.InlineKeyboardButton("🔙 К списку групп", callback_data="group_statistics"))
    markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))
...
def all_numbers(call):
    ...
    try:
        with db.get_db() as conn:
            ...
            ''')
            numbers = cursor.fetchall()
            print(f"Получено {len(numbers)} номеров")  # Отладка
        ...
        # Сохраняем данные в кэш для пагинации
        user_id = call.from_user.id
        numbers_data_cache[user_id] = numbers
        
        # Показываем первую страницу
        show_numbers_page(call, 0)
    
    except Exception as e:
        print(f"Ошибка в all_numbers: {e}")
        bot.answer_callback_query(call.id, "❌ Произошла ошибка при загрузке номеров!")
...
def show_pending_requests(call):
    ...
    if not requests:
        ...
        for user_id, last_request in requests:
            try:
                user = bot.get_chat_member(user_id, user_id).user
                username = f"@{user.username}" if user.username else "Нет username"
            except:
    ...
    try:
        if not message.reply_to_message or \
           ...
           message.reply_to_message.message_id != original_message_id:
            ...
            msg = bot.send_message(
                message.chat.id,
                f"❌ Пожалуйста, ответьте на исходное сообщение с запросом кода !",
                reply_markup=markup
            )
            bot.register_next_step_handler(
                msg,
        ...
        if not user_input:
            ...
            msg = bot.send_message(
                message.chat.id,
                f"❌ Код не может быть пустым! Введите код для ТГ {tg_number} (ответьте на исходное сообщение):",
                reply_markup=markup
            )
        ...
        markup.add(
            types.InlineKeyboardButton(
                "✅ Да, код верный",
                callback_data=f"confirm_code_{number}_{user_input}_{group_chat_id}_{message.chat.id}_{confirmation_msg.message_id}_{tg_number}"
            ),
        ...
        msg = bot.send_message(
            message.chat.id,
            f"❌ Произошла ошибка! Попробуйте ответить на сообщение еще раз (ТГ {tg_number}):",
            reply_markup=markup
        )
        def delete_error_message():
            try:
...
def back_to_confirm(call):
    try:
        ...
        # Извлекаем номер, взяв всё после "back_to_confirm_"
        prefix = "back_to_confirm_"
        if not call.data.startswith(prefix):
            bot.answer_callback_query(call.id, "❌ Неверный формат данных!")
            return
        
        number = call.data[len(prefix):]  # Берем всё после префикса
        ...
        with db.get_db() as conn:
            ...
            # Если код уже отправлен владельцем
            if code and take_date != "0":
                # Формируем сообщение "Встал ли номер?"
                markup = types.InlineKeyboardMarkup()
                markup.add(
                ...
                # Если код ещё не отправлен или номер в состоянии ожидания
                if owner_id:
                    if take_date == "0":
                        # Если модератор ещё не запросил код
                        markup = types.InlineKeyboardMarkup()
                        markup.add(
                            types.InlineKeyboardButton("✉️ Отправить код", callback_data=f"send_code_{number}_{call.message.chat.id}"),
                            types.InlineKeyboardButton("❌ Номер невалидный", callback_data=f"invalid_{number}")
                        ...
                        # Если код уже запрошен, но ещё не получен
                        try:
                            markup = types.ReplyKeyboardRemove()
                            msg = bot.send_message(
                                owner_id,
                            ...
                            with db.get_db() as conn:
                                ...
                                cursor.execute('UPDATE numbers SET VERIFICATION_CODE = "" WHERE NUMBER = ?', (number,))
                                cursor.execute('UPDATE numbers SET MODERATOR_ID = ?, GROUP_CHAT_ID = ? WHERE NUMBER = ?', 
                                            (call.from_user.id, call.message.chat.id, number))
                                conn.commit()
                    ...
                    # Если владелец не найден, но номер существует
                    markup = types.InlineKeyboardMarkup()
                    markup.add(types.InlineKeyboardButton("📲 Получить новый номер", callback_data="get_number"))
                    markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        f"❌ Владелец номера {number} не найден. Номер больше недоступен.\nПожалуйста, получите новый номер.",
                        reply_markup=markup
                    )
